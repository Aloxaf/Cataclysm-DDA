...
def extract_achievement(a):
    ...
    for f in ("name", "description"):
        if f in a:
            a[f] = writestr(outfile, a[f])
    for req in a.get("requirements", ()):
        if "description" in req:
            req["description"] = writestr(outfile, req["description"])
...
def extract_bodypart(item):
    ...
    item["name"] = writestr(outfile, item["name"])
    if "name_multiple" in item:
        item["name_multiple"] = writestr(outfile, item["name_multiple"])
    item["accusative"] = writestr(outfile, item["accusative"])
    if "accusative_multiple" in item:
        item["accusative_multiple"] = writestr(outfile, item["accusative_multiple"])
    item["encumbrance_text"] = writestr(outfile, item["encumbrance_text"])
    item["heading"] = writestr(outfile, item["heading"])
    item["heading_multiple"] = writestr(outfile, item["heading_multiple"])
    if "hp_bar_ui_text" in item:
        item["hp_bar_ui_text"] = writestr(outfile, item["hp_bar_ui_text"])


def extract_clothing_mod(item):
    outfile = get_outfile("clothing_mod")
    item["implement_prompt"] = writestr(outfile, item["implement_prompt"])
    item["destroy_prompt"] = writestr(outfile, item["destroy_prompt"])


def extract_construction(item):
    outfile = get_outfile("construction")
    if "pre_note" in item:
        item["pre_note"] = writestr(outfile, item["pre_note"])


def extract_harvest(item):
    outfile = get_outfile("harvest")
    if "message" in item:
        item["message"] = writestr(outfile, item["message"])
...
def extract_material(item):
    ...
    if "bash_dmg_verb" in item:
        item["bash_dmg_verb"] = writestr(outfile, item["bash_dmg_verb"])
    if "cut_dmg_verb" in item:
        item["cut_dmg_verb"] = writestr(outfile, item["cut_dmg_verb"])
    if "dmg_adj" in item:
        item["dmg_adj"][0] = writestr(outfile, item["dmg_adj"][0])
        item["dmg_adj"][1] = writestr(outfile, item["dmg_adj"][1])
        item["dmg_adj"][2] = writestr(outfile, item["dmg_adj"][2])
        item["dmg_adj"][3] = writestr(outfile, item["dmg_adj"][3])
...
def extract_martial_art(item):
    ...
    if "description" in item:
        item["description"] = writestr(outfile, item["description"])
    if "initiate" in item:
        item["initiate"] = writestr(outfile, item["initiate"])
    ...
    for buff in buffs:
        buff["name"] = writestr(outfile, buff["name"])
        buff["description"] = writestr(outfile, buff["description"])
...
def extract_move_mode(item):
    ...
    item["character"] = writestr(outfile, item["character"])
    # The character in the panels
    item["panel_char"] = writestr(outfile, item["panel_char"])
    # Successful change message
    item["change_good_none"] = writestr(outfile, item["change_good_none"])
    # Successful change message (animal steed)
    item["change_good_animal"] = writestr(outfile, item["change_good_animal"])
    # Successful change message (mech steed)
    item["change_good_mech"] = writestr(outfile, item["change_good_mech"])
    if "change_bad_none" in item:
        # Failed change message
        item["change_bad_none"] = writestr(outfile, item["change_bad_none"])
    if "change_bad_animal" in item:
        # Failed change message (animal steed)
        item["change_bad_animal"] = writestr(outfile, item["change_bad_animal"])
    if "change_bad_mech" in item:
        # Failed change message (mech steed)
        item["change_bad_mech"] = writestr(outfile, item["change_bad_mech"])
...
def extract_gun(item):
    ...
    if "name" in item:
        if item["type"] in needs_plural:
            item["name"] = writestr(outfile, item["name"], pl_fmt=True)
        else:
            item["name"] = writestr(outfile, item["name"])
    if "description" in item:
        item["description"] = writestr(outfile, item["description"])
    if "modes" in item:
        ...
        for fire_mode in modes:
            fire_mode[1] = writestr(outfile, fire_mode[1])
    if "skill" in item:
        ...
        if not item["skill"] == "archery":
            item["skill"] = writestr(outfile, item["skill"], context="gun_type_type")
    if "reload_noise" in item:
        item["reload_noise"] = writestr(outfile, item["reload_noise"])
    return item
...
def extract_gunmod(item):
    ...
    if "mode_modifier" in item:
        ...
        for fire_mode in modes:
            fire_mode[1] = writestr(outfile, fire_mode[1])
    if "location" in item:
        item["location"] = writestr(outfile, item["location"])
...
def extract_professions(item):
    ...
    nm = item["name"]
    if type(nm) == dict:
        nm["male"] = writestr(outfile, nm["male"], context="profession_male")
        item["description"] = writestr(outfile, item["description"], context="prof_desc_male")

        nm["female"] = writestr(outfile, nm["female"], context="profession_female")
        item["description"] = writestr(outfile, item["description"], context="prof_desc_female")
    else:
        item["name"] = writestr(outfile, nm, context="profession_male")
        item["description"] = writestr(outfile, item["description"], context="prof_desc_male")

        item["name"] = writestr(outfile, nm, context="profession_female")
        item["description"] = writestr(outfile, item["description"], context="prof_desc_female")
    return item
...
def npgettext(context, single, plural):
    if context:
        if plural:
            return zh_CN.npgettext(context, single, plural, 3)
        else:
            return zh_CN.pgettext(context, single)
    else:
        if plural:
            return zh_CN.ngettext(single, plural, 3)
        else:
...
def writestr(filename, string, context=None, format_strings=False,
    ...
    if type(string) is list:
        return [writestr(filename, entry, context, format_strings, comment, pl_fmt) for entry in string]
        ...
        if "str_pl" in string:
            string["str_pl"] = npgettext(context, None, string["str_pl"])
        if "str" in string:
            string["str"] = npgettext(context, string["str"], None)
        elif "str_sp" in string:
            string["str_sp"] = npgettext(context, None, string["str_sp"])
        else:
...
def extract_use_action_msgs(outfile, use_action, it_name, kwargs):
    ...
    for f in sorted(use_action_msgs):
        if type(use_action) is dict and f in use_action:
            if it_name:
                use_action[f] = writestr(outfile, use_action[f], **kwargs)
    # Recursively check sub objects as they may contain more messages.
    if type(use_action) is list:
        return [extract_use_action_msgs(outfile, i, it_name, kwargs) for i in use_action]
    elif type(use_action) is dict:
        return {k: extract_use_action_msgs(outfile, v, it_name, kwargs) for (k, v) in sorted(use_action.items(), key=lambda x: x[0])}
...
def extract(item, infilename):
    ...
    if name:
        if object_type in needs_plural:
            item["name"] = writestr(outfile, name, pl_fmt=True, **kwargs)
        else:
            item["name"] = writestr(outfile, name, **kwargs)
    if "name_suffix" in item:
        item["name_suffix"] = writestr(outfile, item["name_suffix"], **kwargs)
    if "name_unique" in item:
        item["name_unique"] = writestr(outfile, item["name_unique"], **kwargs)
    if "job_description" in item:
        item["job_description"] = writestr(outfile, item["job_description"], **kwargs)
    if "use_action" in item:
        item["use_action"] = extract_use_action_msgs(outfile, item["use_action"], item.get("name"),
                                kwargs)
    if "conditional_names" in item:
        for cname in item["conditional_names"]:
            ...
            wrote = True
    if "description" in item:
        item["description"] = writestr(outfile, item["description"], **kwargs)
    if "detailed_definition" in item:
        item["detailed_definition"] = writestr(outfile, item["detailed_definition"], **kwargs)
    if "sound" in item:
        item["sound"] = writestr(outfile, item["sound"], **kwargs)
    if "sound_description" in item:
        item["sound_description"] = writestr(outfile, item["sound_description"], **kwargs)
    if "snippet_category" in item and type(item["snippet_category"]) is list:
        ...
        for entry in item["snippet_category"]:
            ...
            if type(entry) is dict:
                entry["text"] = writestr(outfile, entry["text"], **kwargs)
            else:
                # or a simple string
                # TODO: 
    ...
    if "bash" in item and type(item["bash"]) is dict:
        ...
        bash = item["bash"]
        if "sound" in bash:
            bash["sound"] = writestr(outfile, bash["sound"], **kwargs)
        if "sound_fail" in bash:
            bash["sound_fail"] = writestr(outfile, bash["sound_fail"], **kwargs)
    if "seed_data" in item:
        seed_data = item["seed_data"]
        seed_data["plant_name"] = writestr(outfile, seed_data["plant_name"], **kwargs)
    if "relic_data" in item and "name" in item["relic_data"]:
        item["relic_data"]["name"] = writestr(outfile, item["relic_data"]["name"], **kwargs)
    if "text" in item:
        item["text"] = writestr(outfile, item["text"], **kwargs)
    if "message" in item:
        item["message"] = writestr(outfile, item["message"], format_strings=True, **kwargs)
    if "messages" in item:
        item["messages"] = [writestr(outfile, message, **kwargs) for message in item["messages"]]
    if "valid_mod_locations" in item:
        for mod_loc in item["valid_mod_locations"]:
            writestr(outfile, mod_loc[0], **kwargs)
            wrote = True
    if "info" in item:
        item["info"] = writestr(outfile, item["info"], **kwargs)
    if "restriction" in item:
        item["restriction"] = writestr(outfile, item["restriction"], **kwargs)
    if "verb" in item:
        item["verb"] = writestr(outfile, item["verb"], **kwargs)
    ...
    if "footsteps" in item:
        item["footsteps"] = writestr(outfile, item["footsteps"], **kwargs)
    if "revert_msg" in item:
        item["revert_msg"] = writestr(outfile, item["revert_msg"], **kwargs)
